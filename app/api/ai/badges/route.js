...
export async function POST(request) {
  ...
  const { stats = {}, personality = 'default' } = await request.json()
  const hint = PERSONALITY_HINTS[personality] || PERSONALITY_HINTS.default

  const {
    totalCompleted = 0, daysSinceJoin = 0, activeDays30 = 0, activeDays7 = 0,
    peakHour = null, topTags = [], highPriDone = 0, totalTags = 0,
    streak = 0, delayAvg = 0, weeklyCompletions = [],
  } = stats
...
`
  ...
  try {
    ...
    if (!result?.badges?.length)
      return Response.json({ badges: [{ icon: '🌱', label: '성장 중', desc: '꾸준히 가는 중' }] })
    ...
    return Response.json({ badges: clean })
  } catch (e) {
